...
import { Add, LibraryBooks, PlayArrow } from "@mui/icons-material";
...
const mapToLangMarkdown = (l: string) => {
    ...
    for (let i = 0; i < languages.length; i++) {
        ...
        for (let j = 0; j < languages[i].extensions.length; j++) {
            if (l.toLowerCase() === languages[j].extensions[j]) {
                return languages[i].extensions[0]
            }
        }
    }
...
function HomeworkHelper({params}: { params: { id: string } }) {
    const authState = useAppSelector(selectAuthState);
    ...
    const [newChat, setNewChat] = React.useState(localStorage.getItem("hh_new") === "true");
    ...
    useEffect(() => {
        console.log("inside sub page render", id, newChat, messages)
        if (id === "-1" || !newChat || messages.length !== 0)
            // console.log("is not new chat: ", newChat, messages)
            return

        let rawLoadedCode: null | string = localStorage.getItem("code")
        let loadedCode: CtCodeFile[] = rawLoadedCode !== null ? JSON.parse(rawLoadedCode) : []
        let rawLoadedMessages: null | string = localStorage.getItem("user_message")
        console.log("loadedMessages", rawLoadedMessages)
        let loadedMessages: CtGetHHChatMessagesResponseMessage[] = rawLoadedMessages !== null ? JSON.parse(rawLoadedMessages) : []
        let workspaceId = localStorage.getItem("workspace_id")

        // clear the local storage for the next chat
        localStorage.removeItem("code")
        localStorage.removeItem("user_message")
        localStorage.removeItem("hh_new")
        localStorage.removeItem("workspace_id")

        setSelectedChat(id)
        setMessages(loadedMessages)
        console.log("setting messages in sub page render: ", messages)
        setCode(loadedCode)
        setActiveFile(loadedCode.length > 0 ? loadedCode[0].file_name : "")
        setEditorOpen(loadedCode.length > 0)
        ...
        sendUserMessage(id, loadedMessages[0].content, false)
    }, [])
    ...
    useEffect(() => {
        ...
        ctWs.sendWebsocketMessage({
            ...
            (msg: CtMessage<CtGenericErrorPayload | CtValidationErrorPayload | CtGetHHChatMessagesResponse>): boolean => {
                ...
                console.log("setting messages: ", res.messages)
    ...
    const sendUserMessage = async (chatId: string, userMessage: string, addMessage: boolean = true, codeExec: boolean = false) => {
        console.log("inside sendUserMessage")
        if (addMessage) {
    ...
    const sendExecRequest = (code: CtCodeFile[], activeFile: string, retryCount: number = 0) => {
        ...
        const message = {
            sequence_id: Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15),
    ...
    const renderEditor = () => {
        ...
        let stateIcon = (<LinkOffIcon sx={{ color: theme.palette.text.primary + "60" }} />)
                            ...
                            <HomeworkHelperActive
                                ...
                                sendMessage={(content: string) => {
                                    if (selectedChat === null || selectedChat === "-1" || content === "") {
                                        console.log("exiting because selectedChat is: ", selectedChat)
                                        return
                                    }
                                    console.log("inside sendMessage global")
                                    setActiveResponse({ text: "", files: code, command: null });
...
HomeworkHelper.disableStrictMode = true;
