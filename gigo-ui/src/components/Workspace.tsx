...
import LaunchPadRocketIcon from "@/icons/launch-pad-rocket";
...
const WorkspacePage = ({ params }: { params: { id: string } }) => {
    ...
    const setHighestScoreFunc = async (num: any) => {
        let res = await fetch(
            `${config.rootPath}/api/workspace/setHighestScore`,
            {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        const updateLeftOpen = () => {
            setLeftOpen(query.get("menu") === "true");
        };

        const updateRightOpen = () => {
            setRightOpen(query.get("chat") === "true");
        };
        ...
        const calculateLeftPosition = () => {
            if (leftOpen) {
                return aspectRatio === '21:9' ? '40.5vw' : '51vw';
            } else if (rightOpen) {
        ...
        console.log("set left: ", calculateLeftPosition());
    }, [query, leftOpen, rightOpen]);
