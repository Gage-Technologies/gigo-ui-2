...
function RenderQuizPageMobile({ data }: QuizPageProps) {
  ...
  const renderQuestion = () => {
    return (
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, width: '100%', mt: 4 }}>
          <Typography variant="h5" component="h1" sx={{ mt: 2, textAlign: 'center' }}>
            {data.name}
          </Typography>

          {/* Progress bar and question count */}
          <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 1 }}>
            <Box sx={{ flexGrow: 1, bgcolor: 'grey.300', height: 8, borderRadius: 4 }}>
              <Box
                sx={{
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr', gap: 2, width: '100%' }}>
                {currentQuestion.options.map((answer, index) => (
                  <Card
                    ...
                    onClick={() => handleAnswerClick(index)}
                  >
                    <Typography variant="body2" sx={{ fontSize: '0.9rem', textAlign: 'center', padding: '10px' }}>{answer}</Typography>
                  </Card>
                ))}
              </Box>
            </Box>
...
export default RenderQuizPageMobile;